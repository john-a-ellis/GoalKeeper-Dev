### Custom Classes and Methods based on Neo4j Source Code
...
class SingleEntitySinglePropertyExactMatchResolver(EntityResolver):
    """Resolve specific entities with same label and exact same property (default is "name").

    Args:
        driver (neo4j.Driver): The Neo4j driver to connect to the database.
        resolve_node_type (Optional[str]): To reduce the resolution scope, add a specific node label.
    ...
    .. code-block:: python
        ...
        from neo4j_graphrag.experimental.components.resolver import SingleEntitySinglePropertyExactMatchResolver
    ...
    def __init__(
        self,
        driver: neo4j.Driver,
        resolve_node_type: Optional[str] = None,
        filter_query: Optional[str] = None,
        resolve_property: str = "name",
        neo4j_database: Optional[str] = None,
    ) -> None:
        super().__init__(driver, filter_query)
        self.resolve_node_type = resolve_node_type
    ...
    async def run(self) -> ResolutionStats:
        ...
        """
        if self.resolve_node_type:
            match_query = f"MATCH (entity:{self.resolve_node_type}) "
        else:            
