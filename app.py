...
Analyze the following document and identify any of the following as nodes: {{allowed_nodes}} if they are contained in the text,
...
AI messages to the 'AI' entity as appropriate.
...
def switch_tab(active_tab, stored_response, stored_context, stored_chat_history, stored_entities, stored_summary):
    ...
    if triggered_id in ['tabs', 'store-response', 'store-context', 'store-chat-history','store-entity-memory']:
        if active_tab == "tab-response":
            ...
            return dbc.Card(dbc.CardBody([dcc.Markdown(this)], className="card-context")), no_update, no_update
    ...
    html.Label("Selected Neo4J Network Relationships", "edge-label"),
    dcc.Dropdown(
    ...
    cyto.Cytoscape(
        ...
        style={'width': '100%', 'height': '550px', 'background-color': 'aliceblue'}
    ),
